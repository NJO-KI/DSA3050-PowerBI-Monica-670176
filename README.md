...
- **Key Questions Answered:**
  ...
  2. Which shipping modes produce the highest delay risk?
  3. How does revenue compare Year-over-Year (YoY)?
  4. Which regions suffer from severe fulfillment bottlenecks?
  5. What root causes drive late delivery flags?
...
1. **Customer Name Formatting**
   ...
   - **Transformation:** Selected both columns -> `Transform` -> `Merge Columns` (Separator: Space) -> Renamed to `Customer Name`.
...
4. **Standardize Delivery Status Categories**
   - **Problem:** `Delivery Status` contained inconsistent values such as `SUSPECTED_FRAUD`, `LATE_DELIVERY`, and `CANCELED`.
...
6. **Text Trimming & Cleaning**
   - **Problem:** `Market` and `Order Region` strings contained leading and trailing whitespace.
   - **Transformation:** Selected text dimension columns -> `Transform` -> `Text Column` -> `Trim` and `Clean`.
...
7. **Dimensional Table Creation via Referencing**
   - **Problem:** Raw dataset was a single flat table containing customer, product, and location attributes mixed with measures.
   ...
   - **Result:** Isolated `DimCustomer` table with unique `Customer Id` primary keys.
...
<img width="1340" height="750" alt="image" src="https://github.com/user-attachments/assets/9fa003f4-1bda-4def-be6b-170031bfcd5a" />

* **Analytical Focus:** High-level executive monitoring of global supply chain health, volume performance, and shipping efficiency.
* **Key Visuals:**
  * **KPI Ribbon:** Tracks top-line metrics (`Total Revenue`, `Total Orders`, `OTDR %`, `Late Delivery Risk %`, `Logistics Health Status`).
  * **Line & Clustered Column Chart:** Highlights correlation between total monthly sales and late delivery risk rates.
  * **Filled Map:** Maps country-level revenue density with on-time performance tooltips.
  * **Donut Chart:** Measures order volume distribution across customer segments.

### Detailed Logistics Analysis

<img width="1365" height="821" alt="image" src="https://github.com/user-attachments/assets/61a21f48-db04-49b9-8369-433d800949d6" />

* **Analytical Focus:** Evaluates carrier efficiency, regional shipment performance, and product category bottlenecks.
* **Key Visuals:**
  * **Market Matrix:** Enables drill-down from `Market` to `Order Region` across all shipping modes.
  * **Top Delayed Categories:** Isolates top product lines generating the highest delay volume.
  * **Margin vs. Lead Time Scatter:** Detects regional profit margin erosion against shipping duration.

### Diagnostic Insights

<img width="1334" height="800" alt="image" src="https://github.com/user-attachments/assets/bb093ea2-9a6b-40fd-a6e6-33a707ce2b69" />

* **Analytical Focus:** Root-cause investigation into late shipping drivers and profit leakage.
* **Key Visuals:**
  * **Decomposition Tree:** Deconstructs `Late Delivery Risk %` across regional, operational, and customer tiers.
  * **Key Influencers Visual:** Uses machine learning models to highlight the top statistical drivers of high delivery risks.
  * **Waterfall Chart:** Maps financial flow from gross revenue down to net order profit.
